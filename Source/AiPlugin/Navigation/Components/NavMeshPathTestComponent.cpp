...
void plAiNavMeshPathTestComponent::Update()
{
  ...
  {
    m_Navigation.DebugDrawPathCorridor(GetWorld(), plColor::Aquamarine.WithAlpha(0.2f));
  }

  if (m_bVisualizePathLine)
  {
    m_Navigation.DebugDrawPathLine(GetWorld(), plColor::Lime);
  }
...
PL_STATICLINK_FILE(AiPlugin, AiPlugin_Navigation_Components_NavMeshPathTestComponent);