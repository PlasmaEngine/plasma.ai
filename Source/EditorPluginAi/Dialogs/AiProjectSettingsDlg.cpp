...
void plQtAiProjectSettingsDlg::SaveState()
{
  ...
  RetrieveTacticalConfig();
  RetrieveSquadConfig();
  m_Config.Save().IgnoreResult();
  UpdateGroundTypeDynamicEnumValues();
}

void plQtAiProjectSettingsDlg::ApplySquadConfig()
{
  const auto& sc = m_Config.m_SquadConfig;

  SquadAttackTokens->setValue(sc.m_uiAttackTokens);
  SquadShareConfidence->setValue(sc.m_fShareConfidence);
  SquadShareStrength->setValue(sc.m_fShareStrength);
  SquadShareInterval->setValue(sc.m_fShareInterval);
}

void plQtAiProjectSettingsDlg::RetrieveSquadConfig()
{
  auto& sc = m_Config.m_SquadConfig;

  sc.m_uiAttackTokens = (plUInt8)SquadAttackTokens->value();
  sc.m_fShareConfidence = (float)SquadShareConfidence->value();
  sc.m_fShareStrength = (float)SquadShareStrength->value();
  sc.m_fShareInterval = (float)SquadShareInterval->value();
}

void plQtAiProjectSettingsDlg::FillTacticalNavmeshComboBox()
{
  plQtScopedBlockSignals _1(TacCoverNavmesh);

  const QString sCurrent = TacCoverNavmesh->currentData().toString();

  TacCoverNavmesh->clear();
  TacCoverNavmesh->addItem("<first navmesh>", QVariant(QString()));

  for (const auto& cfg : m_Config.m_NavmeshConfigs)
  {
    TacCoverNavmesh->addItem(cfg.m_sName.GetData(), QVariant(QString(cfg.m_sName.GetData())));
  }

  const int iIndex = TacCoverNavmesh->findData(QVariant(sCurrent));
  TacCoverNavmesh->setCurrentIndex(iIndex >= 0 ? iIndex : 0);
}

void plQtAiProjectSettingsDlg::ApplyTacticalConfig()
{
  FillTacticalNavmeshComboBox();

  const auto& tc = m_Config.m_TacticalConfig;

  const int iIndex = TacCoverNavmesh->findData(QVariant(QString(tc.m_sCoverNavmesh.GetData())));
  TacCoverNavmesh->setCurrentIndex(iIndex >= 0 ? iIndex : 0);

  TacCoverSpacing->setValue(tc.m_fCoverSpacing);
  TacCoverProbeDistance->setValue(tc.m_fCoverProbeDistance);
  TacCoverCrouchHeight->setValue(tc.m_fCoverCrouchHeight);
  TacCoverStandHeight->setValue(tc.m_fCoverStandHeight);
  TacMaxCoverPerSector->setValue(tc.m_uiMaxCoverPointsPerSector);
}

void plQtAiProjectSettingsDlg::RetrieveTacticalConfig()
{
  auto& tc = m_Config.m_TacticalConfig;

  tc.m_sCoverNavmesh = TacCoverNavmesh->currentData().toString().toUtf8().data();
  tc.m_fCoverSpacing = (float)TacCoverSpacing->value();
  tc.m_fCoverProbeDistance = (float)TacCoverProbeDistance->value();
  tc.m_fCoverCrouchHeight = (float)TacCoverCrouchHeight->value();
  tc.m_fCoverStandHeight = (float)TacCoverStandHeight->value();
  tc.m_uiMaxCoverPointsPerSector = (plUInt16)TacMaxCoverPerSector->value();
}
...
{
  plUInt32 cur = SelectedPathCfg->currentIndex();
  ...
  FillPathSearchTypeComboBox();
  SelectedPathCfg->setCurrentIndex(plMath::Min(cur, (plUInt32)SelectedPathCfg->count() - 1));
}
...
{
  plUInt32 cur = SelectedMeshCfg->currentIndex();
  ...
  FillNavmeshTypeComboBox();
  SelectedMeshCfg->setCurrentIndex(plMath::Min(cur, (plUInt32)SelectedMeshCfg->count() - 1));
}
...
void plQtAiProjectSettingsDlg::FillNavmeshTypeComboBox()
{
  ...
  // navmesh list changed - keep the tactical cover-navmesh choices in sync
  FillTacticalNavmeshComboBox();
}
